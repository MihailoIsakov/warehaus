'use strict';
 ...
 angular.module('artikliModal', ['angular-md5'])

 .controller('artikliModalCtrl', function (PraviArtikal, $scope, $routeParams,$route, $modal, $log, $location, $modalInstance ) {
...
if($routeParams.artikalId !='new'){
		...
		modalInstance.result.then(function (data) {
			...
			if( data.action==='save'){
				selectedArtikal.$create(function () {
					$scope.artikliDoc.push(selectedArtikal);
			},
            function (response) {
	...
	$scope.delete = function () {
		...
		PraviArtikal.delete({'idArtikal':$scope.selectedArtikal.idArtikal},function () {
			var index = $scope.artikliDoc.indexOf($scope.selectedArtikal);
			$scope.artikliDoc.splice(index,1);
			});
	}
